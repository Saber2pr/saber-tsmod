...
function createModuleFile($name) {
    return __awaiter(this, void 0, void 0, function () {
        var rename, _a, str, type, name, moduleFilePath, moduleFileContent, moduleTest, moduleTestContent, test_entry, module_test, export_entry, export_entry_content;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    rename = rule_1.Name.reset($name);
                    _a = saber_node_1.Path.split($name), str = _a.str, type = _a.type;
                    name = str;
                    moduleFilePath = path_config_1.path_core + "/" + name + "." + type;
                    moduleFileContent = module_1.module(name, rename).core;
                    if (saber_node_1.Path.isExist(moduleFilePath)) {
                ...
                case 1:
                    // create module file
                    _b.sent();
                    moduleTest = path_config_1.path_test + "/" + name + "." + type;
                ...
                case 2:
                    // create module test file
                    _b.sent();
                    test_entry = path_config_1.path_test + "/test." + type;
                    module_test = "import { test_" + rename + " } from './" + name + "'\ntest_" + rename + "()\n\n";
                    if (!!saber_node_1.Path.isExist(test_entry)) return [3 /*break*/, 4];
                    return [4 /*yield*/, saber_node_1.File.createFile(test_entry, module_test)];
                case 3:
                    _b.sent();
                    return [3 /*break*/, 6];
                case 4: return [4 /*yield*/, saber_node_1.File.push(test_entry, module_test)];
                case 5:
                    _b.sent();
                    _b.label = 6;
                case 6:
                    export_entry = path_config_1.path_root + "/index." + type;
                    export_entry_content = "export * from './core/" + name + "'\n";
                    if (!!saber_node_1.Path.isExist(export_entry)) return [3 /*break*/, 8];
                    return [4 /*yield*/, saber_node_1.File.createFile(export_entry, export_entry_content)];
                case 7:
                    _b.sent();
                    return [3 /*break*/, 10];
                case 8: return [4 /*yield*/, saber_node_1.File.push(export_entry, export_entry_content)];
                case 9:
                    _b.sent();
                    _b.label = 10;
                case 10:
